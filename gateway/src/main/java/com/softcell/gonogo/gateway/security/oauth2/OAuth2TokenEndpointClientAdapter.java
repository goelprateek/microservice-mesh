...
public abstract class OAuth2TokenEndpointClientAdapter implements OAuth2TokenEndpointClient {
    ...
    protected final GoNoGoProperties goNoGoProperties;
    protected final OAuth2Properties oAuth2Properties;
    private final Logger LOGGER = LoggerFactory.getLogger(OAuth2TokenEndpointClientAdapter.class);

    public OAuth2TokenEndpointClientAdapter(RestTemplate restTemplate, GoNoGoProperties goNoGoProperties, OAuth2Properties oAuth2Properties) {
    ...
    public OAuth2AccessToken sendPasswordGrant(String username, String password) {
        ...
        LOGGER.debug("contacting OAuth2 token endpoint to login user: {}", username);
        ...
        if (null != responseEntity && responseEntity.getStatusCode() != HttpStatus.OK) {
            LOGGER.debug("failed to authenticate user with OAuth2 token endpoint, status: {}", responseEntity.getStatusCodeValue());
    ...
    public OAuth2AccessToken sendRefreshGrant(String refreshTokenValue) {
        ...
        LOGGER.debug("contacting OAuth2 token endpoint to refresh OAuth2 JWT tokens");
        ResponseEntity<OAuth2AccessToken> responseEntity = restTemplate.postForEntity(getTokenEndpoint(), entity,
                OAuth2AccessToken.class);
        if (responseEntity.getStatusCode() != HttpStatus.OK) {
            LOGGER.debug("failed to refresh tokens: {}", responseEntity.getStatusCodeValue());
            throw new HttpClientErrorException(responseEntity.getStatusCode());
        }
        OAuth2AccessToken accessToken = responseEntity.getBody();
        LOGGER.info("refreshed OAuth2 JWT cookies using refresh_token grant");
        return accessToken;
    }
    ...
    protected String getTokenEndpoint() {
        String tokenEndpointUrl = goNoGoProperties.getSecurity().getClientAuthorization().getAccessTokenUri();
