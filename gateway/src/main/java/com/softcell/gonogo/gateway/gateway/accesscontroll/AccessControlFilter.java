package com.softcell.gonogo.gateway.gateway.accesscontroll;
...
public class AccessControlFilter extends ZuulFilter{
    ...
    public AccessControlFilter(RouteLocator routeLocator, GoNoGoProperties goNoGoProperties) {
    ...
    private boolean isAuthorizedRequest(String serviceUrl, String serviceName, String requestUri) {
        ...
        Map<String, List<String>> authorizedMicroservicesEndpoints = goNoGoProperties.getGateway().getAuthorizedMicroservicesEndpoints();
        ...
        if (authorizedMicroservicesEndpoints.get(serviceName) == null) {
            logger.debug("Access Control: allowing access for {}, as no access control policy has been set up for " +
            ...
            for (String endpoint : authorizedEndpoints) {
                ...
                if (requestUri.startsWith(gatewayEndpoint)) {
                    logger.debug("Access Control: allowing access for {}, as it matches the following authorized " +
    ...
    public Object run() {
        ...
        logger.debug("Access Control: filtered unauthorized access on endpoint {}", ctx.getRequest().getRequestURI());
        return null;
    }
}
