...
public class CacheConfiguration {
    ...
    @Bean
    public HazelcastInstance hazelcastInstance(GoNoGoProperties goNoGoProperties) {
        log.debug("Configuring Hazelcast");
        HazelcastInstance hazelCastInstance = Hazelcast.getHazelcastInstanceByName("zuulproxy");
        if (hazelCastInstance != null) {
            log.debug("Hazelcast already initialized");
            return hazelCastInstance;
        }
        Config config = new Config();
        config.setInstanceName("zuulproxy");
        ...
        if (this.registration == null) {
            ...
            if (env.acceptsProfiles(GoNoGoConstants.SPRING_PROFILE_DEVELOPMENT)) {
    ...
    private MapConfig initializeDomainMapConfig(GoNoGoProperties goNoGoProperties) {
        MapConfig mapConfig = new MapConfig();
        mapConfig.setTimeToLiveSeconds(goNoGoProperties.getCache().getHazelcast().getTimeToLiveSeconds());
        return mapConfig;
    }

    private MapConfig initializeClusteredSession(GoNoGoProperties goNoGoProperties) {
        MapConfig mapConfig = new MapConfig();
        mapConfig.setBackupCount(goNoGoProperties.getCache().getHazelcast().getBackupCount());
        mapConfig.setTimeToLiveSeconds(goNoGoProperties.getCache().getHazelcast().getTimeToLiveSeconds());
        return mapConfig;
    }
